// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: resources.proto

package network.minter.grpc;

public interface BlockResponseOrBuilder extends
    // @@protoc_insertion_point(interface_extends:api_pb.BlockResponse)
    com.google.protobuf.MessageOrBuilder {

  /**
   * <code>string hash = 1 [json_name = "hash"];</code>
   * @return The hash.
   */
  java.lang.String getHash();
  /**
   * <code>string hash = 1 [json_name = "hash"];</code>
   * @return The bytes for hash.
   */
  com.google.protobuf.ByteString
      getHashBytes();

  /**
   * <code>uint64 height = 2 [json_name = "height"];</code>
   * @return The height.
   */
  long getHeight();

  /**
   * <code>string time = 3 [json_name = "time"];</code>
   * @return The time.
   */
  java.lang.String getTime();
  /**
   * <code>string time = 3 [json_name = "time"];</code>
   * @return The bytes for time.
   */
  com.google.protobuf.ByteString
      getTimeBytes();

  /**
   * <code>uint64 transaction_count = 4 [json_name = "transactionCount"];</code>
   * @return The transactionCount.
   */
  long getTransactionCount();

  /**
   * <code>repeated .api_pb.TransactionResponse transactions = 6 [json_name = "transactions"];</code>
   */
  java.util.List<network.minter.grpc.TransactionResponse> 
      getTransactionsList();
  /**
   * <code>repeated .api_pb.TransactionResponse transactions = 6 [json_name = "transactions"];</code>
   */
  network.minter.grpc.TransactionResponse getTransactions(int index);
  /**
   * <code>repeated .api_pb.TransactionResponse transactions = 6 [json_name = "transactions"];</code>
   */
  int getTransactionsCount();
  /**
   * <code>repeated .api_pb.TransactionResponse transactions = 6 [json_name = "transactions"];</code>
   */
  java.util.List<? extends network.minter.grpc.TransactionResponseOrBuilder> 
      getTransactionsOrBuilderList();
  /**
   * <code>repeated .api_pb.TransactionResponse transactions = 6 [json_name = "transactions"];</code>
   */
  network.minter.grpc.TransactionResponseOrBuilder getTransactionsOrBuilder(
      int index);

  /**
   * <code>string block_reward = 7 [json_name = "blockReward"];</code>
   * @return The blockReward.
   */
  java.lang.String getBlockReward();
  /**
   * <code>string block_reward = 7 [json_name = "blockReward"];</code>
   * @return The bytes for blockReward.
   */
  com.google.protobuf.ByteString
      getBlockRewardBytes();

  /**
   * <code>uint64 size = 8 [json_name = "size"];</code>
   * @return The size.
   */
  long getSize();

  /**
   * <code>string proposer = 9 [json_name = "proposer"];</code>
   * @return The proposer.
   */
  java.lang.String getProposer();
  /**
   * <code>string proposer = 9 [json_name = "proposer"];</code>
   * @return The bytes for proposer.
   */
  com.google.protobuf.ByteString
      getProposerBytes();

  /**
   * <code>repeated .api_pb.BlockResponse.Validator validators = 10 [json_name = "validators"];</code>
   */
  java.util.List<network.minter.grpc.BlockResponse.Validator> 
      getValidatorsList();
  /**
   * <code>repeated .api_pb.BlockResponse.Validator validators = 10 [json_name = "validators"];</code>
   */
  network.minter.grpc.BlockResponse.Validator getValidators(int index);
  /**
   * <code>repeated .api_pb.BlockResponse.Validator validators = 10 [json_name = "validators"];</code>
   */
  int getValidatorsCount();
  /**
   * <code>repeated .api_pb.BlockResponse.Validator validators = 10 [json_name = "validators"];</code>
   */
  java.util.List<? extends network.minter.grpc.BlockResponse.ValidatorOrBuilder> 
      getValidatorsOrBuilderList();
  /**
   * <code>repeated .api_pb.BlockResponse.Validator validators = 10 [json_name = "validators"];</code>
   */
  network.minter.grpc.BlockResponse.ValidatorOrBuilder getValidatorsOrBuilder(
      int index);

  /**
   * <code>.api_pb.BlockResponse.Evidence evidence = 13 [json_name = "evidence"];</code>
   * @return Whether the evidence field is set.
   */
  boolean hasEvidence();
  /**
   * <code>.api_pb.BlockResponse.Evidence evidence = 13 [json_name = "evidence"];</code>
   * @return The evidence.
   */
  network.minter.grpc.BlockResponse.Evidence getEvidence();
  /**
   * <code>.api_pb.BlockResponse.Evidence evidence = 13 [json_name = "evidence"];</code>
   */
  network.minter.grpc.BlockResponse.EvidenceOrBuilder getEvidenceOrBuilder();

  /**
   * <code>repeated string missed = 12 [json_name = "missed"];</code>
   * @return A list containing the missed.
   */
  java.util.List<java.lang.String>
      getMissedList();
  /**
   * <code>repeated string missed = 12 [json_name = "missed"];</code>
   * @return The count of missed.
   */
  int getMissedCount();
  /**
   * <code>repeated string missed = 12 [json_name = "missed"];</code>
   * @param index The index of the element to return.
   * @return The missed at the given index.
   */
  java.lang.String getMissed(int index);
  /**
   * <code>repeated string missed = 12 [json_name = "missed"];</code>
   * @param index The index of the value to return.
   * @return The bytes of the missed at the given index.
   */
  com.google.protobuf.ByteString
      getMissedBytes(int index);

  /**
   * <code>repeated .google.protobuf.Any events = 14 [json_name = "events"];</code>
   */
  java.util.List<com.google.protobuf.Any> 
      getEventsList();
  /**
   * <code>repeated .google.protobuf.Any events = 14 [json_name = "events"];</code>
   */
  com.google.protobuf.Any getEvents(int index);
  /**
   * <code>repeated .google.protobuf.Any events = 14 [json_name = "events"];</code>
   */
  int getEventsCount();
  /**
   * <code>repeated .google.protobuf.Any events = 14 [json_name = "events"];</code>
   */
  java.util.List<? extends com.google.protobuf.AnyOrBuilder> 
      getEventsOrBuilderList();
  /**
   * <code>repeated .google.protobuf.Any events = 14 [json_name = "events"];</code>
   */
  com.google.protobuf.AnyOrBuilder getEventsOrBuilder(
      int index);
}
